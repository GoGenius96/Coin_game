...
list.of.packages <- c("ggplot2", "dplyr", "rmarkdown", "reshape2", "plotly")
...
I once read about a gambling game in a thriller book about a financial meltdown, [Greed - by Marc Elsberg](https://www.amazon.com/-/de/dp/3734105587/ref=tmm_pap_swatch_0?_encoding=UTF8&qid=&sr=). 
It went like this (Spoiler alert): A guy offers the following game to 10 people in a bar. Each of the 10 people pays the guy 10€ and then they individually throw a fair coin 100 times. Each time when they throw a head, their paid money increases by a factor of 1.5 and each time when they throw a tail, their money decreases by a factor of 0.6. After 100 coin tosses they get payed back the amount they ended with.  
...
I was fascinated by this game and was wondering about what went wrong in the reasoning of the man. Therefore I simulated the game and played around with different numbers of players and factors. But I couldn't replicate the conclusion of the guy who offered the game, because indeed, a lot of times, the game was a loss for me (in this scenario I'm the one offering the game). So I began to think, that it doesn't make sense for me to offer this game. But at the same time, most or even all of the players lost the game, so I wouldn't wanna play this game neither. `It seemed to me, that I found a game where it is not recommendable to play the game, but also not recommendable to offer it.`  
...
Let $X_n$ be the a random variable that represents the stake after $n$ rounds. We can describe $X_n$ as follows:
$$ X_n = s\cdot f_w^Z \cdot f_l^{n-Z} = s\cdot f_l^n \cdot (\frac{f_w}{f_l})^Z$$
By applying some transformations we can see that $\log(X_n)$ follows a binomial distribution and is therefore, for large enough n, approximately normal distributed.
...
Log_Games <- function(Startvalue = 10, rounds = 100, players = 50, Win = 1.5, Lose = 0.6, head_prob = 0.5) {
  End_res <- Many_Games(players = players, rounds = rounds, Win = Win, Lose = Lose, head_prob = head_prob) %>%
    mutate(index = 1:(rounds + 1)) %>%
    melt(id.vars = "index", variable.name = "Spieler") %>%
    filter(index == rounds)
...
**Questions:  **
**1. How likely is it to win the game as a player (above red line in Many_Games_plot)?**  
**2. How likely is it to win more than the total stake (above blue line in Many_Games_plot)?**
...
Indeed we can see that $k$ is very sensitive to the ratio of the logarithmic factors. The intuition behind the logarithm here is the fact, that $f_w > 1$ can be infinitely large, but $f_l<1$ is only between 0 and 1. So to compare them appropriately, we take their respective logarithms.
...
*Note*: Because it's hard to see that the last statement is true, here is a clearer explanation:
...
Furthermore $k$ increases with increasing $n$ and $n_p$. The amount of other players has a small impact on the numbers of heads needed to get above the blue line, since we only look at the logarithmic number of players. This can yet again be contributed to the multiplicative effect of the wins and losses and the fact that the total stakes only increase linearly by the number of players. The effect of the number of played rounds is not obvious, because it comes up in several terms.
...
In the following plot we can see the relationships between the number of players and the numbers of played rounds. Note that the z-axis for the probabilities is a log-scaled axis. We can see, that the effect of the number of players decreases very fast and becomes almost constant. The number of played rounds has an almost linear effect on the log-probability of winning. This comes from the fact, that up $\Phi$ depends on $\sqrt{n}$ (up to a scalar-factor) and for large $n$ we can approximate $\sqrt{n}$ by a linear function, i.e. between 200 and 1000 $\sqrt{n}$ looks very linear. 

```{r echo=FALSE}
library(plotly)

rounds <- seq(100, 1000, length = 91)
players <- seq(0, 1000, length = 101)
probabilities <- matrix(nrow = length(rounds), ncol = length(players))

for (p in 1:length(players)) {
  probabilities[, p] <- Winning(rounds = rounds, players = (p - 1) * 20 + 1)
}
plot_ly() %>% add_surface(x = ~players, y = ~rounds, z = ~ log(probabilities))
...
winning_factor <- seq(1, 2, length = 100)
losing_factor <- seq(0.1, 1, length = 200)
probabilities <- matrix(nrow = length(winning_factor), ncol = length(losing_factor))

for (l in 1:length(losing_factor)) {
  probabilities[, l] <- Winning(Win = winning_factor, Lose = losing_factor[l])
}
plot_ly() %>%
  add_surface(y = ~winning_factor, x = ~losing_factor, z = ~probabilities) %>%
  add_trace(x = ~ (2 - winning_factor), y = ~winning_factor, z = 1, mode = "lines", line = list(color = "red", width = 5)) %>%
  add_trace(x = ~ (1 / winning_factor), y = ~winning_factor, z = 1, mode = "lines", line = list(color = "blue", width = 5)) %>%
  add_trace(x = ~ (2 - winning_factor), y = ~winning_factor, z = 0, mode = "lines", line = list(color = "red", width = 5)) %>%
  add_trace(x = ~ (1 / winning_factor), y = ~winning_factor, z = 0, mode = "lines", line = list(color = "blue", width = 5))

```

As expected and explained, the winning probability is very sensitive to $f_w$ and $f_l$. The red lines mark the combinations of $f_w$ and $f_l$ that result in an arithmetic mean of 1. Everything combination of factors in the direction of the yellow area results in an arithmetic mean bigger 1.
The blue lines mark the combinations of $f_w$ and $f_l$ that result in a geometric mean of 1. Everything combination of factors in the direction of the purple area results in a geometric mean smaller 1. As we can see the area between the lines is mostly purple - meaning 0 chance of winning - but at the same time it also contains most of the sensible slopy area, meaning that some combinations of $f_w$ and $f_l$ will not converge fast enough to 0 to make the players certainly lose in 100 rounds. One such combination is $f_w = 1.5$ and $f_l = 0.6$.

# Conclusion

As we can see, the guy from the bar might be doing himself a favour by not offering this game too often, because with increasing an amount of players it becomes very certain that he will lose all his money to a few lucky players. After all, the amount of money that a player can win is very high, but the amount of money a player can lose is just the starting value, i.e. the price of the game. Of course the guy could also choose better values for $f_w$ and $f_l$, but it's not obvious if the game will then still be attractive to players. After all the guy who calculated the average return as $\frac12 1.5 + \frac 12 0.6 = 1.05$ would not play this game if $f_l$ is below $0.5$ or $2-f_w$ in general (red line in the graph above). One combination of $f_w$ and $f_l$ that would satisfy both conditions (geometric mean < 1 and arithmetic mean > 1) is $f_w = 2$ and $f_l = 0.15$. This combination would almost surely lead to a favourable game for the game host, as the winning chances for 100 rounds are `r Winning(Win=2,Lose = 0.15, rounds = 100)`. But at the same time those factors are so ridiculous that probably no one will play. After all most people are very sensitive to loses and losing 85% of the money in a single coin toss is too risky for most people. So instead of adjusting the factors, the guy who offered the game should just stop lecturing the bar guests and go back to study statistics with Dr. Fabian Scheipl.
